...
| POST | `/conversations/ask/stream` | SSE 流式智能问答 |
...
流式接口请求体与同步接口相同，响应类型为 `text/event-stream`：`start` 返回会话 ID，`delta` 返回回答片段，`complete` 返回答案类型、引用、依据状态、缓存状态和耗时等完整元数据。客户端只有收到 `complete` 才应将本轮标记为成功。
